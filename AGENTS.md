...
- **値そのものでなくても、運用の手がかりになる情報。** 社内ツール名、シークレット管理側の item 名
  (`bw get password <item>` の `<item>`)、社内ホスト名、PC の命名規則など。
  値が入っていなくても「何を使っているか」「どこに何という名前で置いてあるか」は公開しない

公開できない個人設定・秘密は dotmine サブモジュール (private) 側に置く。
該当する情報をコミットに含めそうになった場合は、コミットせずユーザーに確認する。

⚠️ **一度 public にコミットしたものは、後から dotmine へ移しても git 履歴からは消えない。**
移してもリモートの履歴・既存の clone・フォークからは取得できる。
「後で移せばいい」ではなく**最初から置き場を判断する**こと。
迷ったら dotmine 側に置く (public へ後から移すのは安全だが、逆は取り消せない)。
